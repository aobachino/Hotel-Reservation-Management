        ...
        <div class="row mt-2 mb-2">
            ...
            <div class="col-lg-6 mb-2">
                <form class="d-flex" method="GET" action="{{ route('customer.index') }}">
                    <input class="form-control me-2" type="search" placeholder="Search by name" aria-label="Search" id="search" name="search" value="{{ request()->input('search') }}">
            ...
            @forelse ($customers as $customer)
                <div class="col-lg-2 col-md-4 col-sm-6 my-1">
                    <div class="card shadow-sm justify-content-start p-0 rounded" style="min-height:350px;">
                            ...
                            <div class="d-flex">
                                ...
                                <div class="dropdown ms-auto mt-2">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                        <li><a class="dropdown-item" href="{{ route('customer.show', ['customer' => $customer->id]) }}">Detail</a></li>
                                        <li><a class="dropdown-item" href="{{ route('customer.edit', ['customer' => $customer->id]) }}">Edit</a></li>
                                        <li>
                                            <form method="POST" id="delete-customer-form-{{ $customer->id }}" action="{{ route('customer.destroy', ['customer' => $customer->id]) }}">
                                                @csrf
                                                @method('DELETE')
                                                <a class="dropdown-item delete" href="#" data-customer-id="{{ $customer->id }}" data-customer-name="{{ $customer->name }}">Delete</a>
                                            </form>
                                        </li>
                                    </ul>
                                </div>
                        ...
                        <img src="{{ $customer->user->getAvatar() }}" style="object-fit: cover; height:350px; border-top-right-radius: 0.5rem; border-top-left-radius: 0.5rem;">
                        <div class="card-body">
                            <div class="card-text">
                                <div class="row">
                                    <div class="col-lg-12">
                                                ...
                                                <div class="table-responsive">
                                                    <table>
                                                        <tr>
                                                            ...
                                                            <td><span>{{ $customer->user->email }}</span></td>
                                                        </tr>
                                                        <tr>
                                                            <td><i class="fas fa-user-md"></i></td>
                                                            <td><span>{{ $customer->job }}</span></td>
                                                        </tr>
                                                        <tr>
                                                            <td><i class="fas fa-map-marker-alt"></i></td>
                                                            <td style="white-space:nowrap" class="overflow-hidden"><span>{{ $customer->address }}</span></td>
                                                        </tr>
                                                        <tr>
                                                            <td><i class="fas fa-phone"></i></td>
                                                            <td><span>+6281233808395</span></td>
                                                        </tr>
                                                        <tr>
                                                            <td><i class="fas fa-birthday-cake"></i></td>
                                                            <td><span>{{ $customer->birthdate }}</span></td>
                                                        </tr>
                                                    </table>
                                                </div>
                                            </div>
        ...
        document.querySelectorAll('.delete').forEach(function(element) {
            element.addEventListener('click', function(event) {
                ...
                swalWithBootstrapButtons.fire({
                    title: 'Are you sure?',
                    text: customerName + " will be deleted, You won't be able to revert this!",
                    icon: 'warning',
